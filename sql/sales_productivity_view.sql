-- CREATE OR REPLACE
-- VIEW public.sales_productivity_vw
-- AS 
WITH follow_up AS (
         WITH latest AS (
SELECT
...
FROM
	...
	SELECT
		...
		ROW_NUMBER() OVER (PARTITION BY follow_up.lead_id
	ORDER BY
		follow_up.follow_up DESC) AS rn
	FROM
		public.follow_up) t
WHERE
	t.rn = 1
...
SELECT
	...
	CASE
		...
		WHEN NOT latest.is_completed
		AND latest.follow_up < now() THEN 'overdue'::text
		ELSE NULL::text
...
RIGHT JOIN (
	...
	FROM
		...
		SELECT
			...
			ROW_NUMBER() OVER (PARTITION BY unt.lead_id
		ORDER BY
			unt.last_activity_created_at DESC) AS rn
		FROM
			(
			SELECT
				l_1.lead_id,
			...
			JOIN (
				...
				WHERE
					...
					AND lower(a.origin::text) = 'outbound'::text
						AND a.call_duration > 0::double PRECISION
                        Or A.activity_type_id=7
						-- OR a.activity_type_id = 4
						-- OR a.activity_type_id = 2
						-- AND (EXISTS (
						-- SELECT
						-- 	1
						-- FROM
						-- 	jsonb_array_elements(a.meta_info -> 'msgs'::text) elem(value)
						-- WHERE
						-- 	(elem.value ->> 'category'::text) = 'Outgoing'::text))
							-- OR a.activity_type_id = 3
							-- AND ((a.meta_info ->> 'status'::text) = 'Pending'::text
							-- 	OR (a.meta_info ->> 'status'::text) = 'Sent'::text)
						GROUP BY
							a.lead_id) aa ON
				l_1.lead_id = aa.lead_id
			JOIN contact c ON
				c.contact_id = l_1.contact_id
			JOIN contact_mapping cm ON
				c.contact_id = cm.contact_id
				AND l_1.rm_id = cm.rm_id
			WHERE
				c.is_active IS TRUE
				AND l_1.is_active IS TRUE
				AND cm.type = 'Sales'::mapping_type
				AND cm.end_date IS NULL
				AND aa.created_at < (now() - '30 days'::INTERVAL)) unt) untouch
	WHERE
		untouch.rn = 1) abc ON
	abc.lead_id = l.lead_id
        ),
appointment_book AS (
         WITH follow_ups AS (
SELECT
	...
	fu_1.completed_on
FROM
	public.follow_up fu_1
WHERE
	fu_1.source = 'whatsapp'::follow_up_source
	AND fu_1.source_info::text <> '{}'::text
                )
SELECT
	DISTINCT ON
	(c.mobile_number,
	c.email_address,
	c.contact_id) c.full_name AS lead_name,
	c.contact_id,
	...
	l.lead_id,
	CASE
		WHEN l.substatus_id = ANY (ARRAY[3,
		6,
		7,
		13,
		14,
	...
	END AS workable_sl,
	CASE
		WHEN l.substatus_id = ANY (ARRAY[9,
		10,
		11,
		15,
		56,
...
FROM
	follow_ups fu
LEFT JOIN LEAD l ON
	fu.contact_id = l.contact_id
LEFT JOIN contact c ON
	c.contact_id = fu.contact_id
LEFT JOIN mst_user mu ON
	mu.user_id = fu.rm_id
LEFT JOIN mst_location ml ON
	ml.location_id = mu.location_id
LEFT JOIN (
	SELECT
		DISTINCT ON
		(ll.lead_id) ll.log_id,
		ll.log_timestamp,
		ll.operation_type,
		ll.lead_id,
		ll.lead_number,
...
LEFT JOIN (
	...
	WHERE
		...
		FROM
			follow_ups))
		AND ll.substatus_id = 3
	ORDER BY
		ll.lead_id,
		ll.log_id DESC) rl ON
	rl.lead_id = fu.lead_id
LEFT JOIN (
	SELECT
		DISTINCT ON
		(sfd_1.lead_id) sfd_1.lead_id,
		sfd_1.payment_date
	FROM
		sales_flat_data sfd_1
	WHERE
		(sfd_1.lead_id IN (
		SELECT
			follow_ups.lead_id
	...
	ORDER BY
		sfd_1.lead_id,
		sfd_1.payment_date) sfd ON
	sfd.lead_id = fu.lead_id
LEFT JOIN (
	SELECT
		DISTINCT ON
		(sl_1.lead_id) sl_1.lead_id,
		sl_1.rm_id,
		sl_1.created_at
	FROM
		status_log sl_1
	WHERE
		...
		FROM
			follow_ups))
		AND sl_1.substatus_id = 29
	ORDER BY
		sl_1.lead_id,
		sl_1.created_at) sl ON
	sl.lead_id = fu.lead_id
LEFT JOIN mst_user mu2 ON
	mu2.user_id = sl.rm_id
GROUP BY
	c.full_name,
	c.contact_id,
	ml.name,
...
FROM
	...
	ORDER BY
		ll.lead_id,
		ll.status_log_id DESC) preparing
        ),
converted_leads AS (
...
FROM
	offline_payment op
WHERE
	op.status = 'Pending'::offline_payment_status
                )
SELECT
	DISTINCT ON
	(pt.lead_id) pt.lead_id,
	pt.payment_date,
	sp.name AS product_name
FROM
	paymenttable pt
LEFT JOIN mst_subproduct sp ON
	pt.sub_product_id = sp.sub_product_id
        )
 SELECT
	base.lead_id,
	base.report_date,
...
FROM
	...
	FROM
		...
		FROM
			mst_user mu
		JOIN mst_department md ON
			md.department_id = mu.department_id
		LEFT JOIN mst_location ml ON
			ml.location_id = mu.location_id
		WHERE
			mu.is_active
		...
		SELECT
			...
			cl.created_at,
			CASE
				WHEN cl.call_duration < 5
					AND ((cl.status_details::text = ANY (ARRAY['NoAnswer'::CHARACTER VARYING::text,
					'NoResponse'::CHARACTER VARYING::text]))
						OR cl.status_details::text = 'NormalCallClearing'::text
						AND cl.call_status::text = 'not_answered'::text
						OR cl.status_details::text = 'NormalUnspecified'::text
						AND cl.call_status::text = 'not_answered'::text) THEN 1
					ELSE NULL::integer
				END AS activity_count_flag
			FROM
				active_users au
			JOIN call_log cl ON
				cl.created_by = au.user_id
			WHERE
				cl.lead_id IS NOT NULL
                        ),
		user_activities AS (
		SELECT
			au.counsellor,
		...
		FROM
			active_users au
		JOIN activity a ON
			a.created_by = au.user_id
			AND a.comment_ids && ARRAY[33::SMALLINT,
			36::SMALLINT,
			61::SMALLINT,
			62::SMALLINT]
		JOIN mst_comment mc ON
			(mc.comment_id = ANY (a.comment_ids))
				AND (mc.comment_id = ANY (ARRAY[33,
				36,
				61,
				62]))
			WHERE
	...
	LEFT JOIN (
		...
		WHERE
			fr.type::text = 'FEEDBACK'::text
			AND fr.assigned_to IS NOT NULL) cs ON
		t2.counsellor_id = cs.assigned_to
		AND t2.lead_id = cs.lead_id
UNION ALL
	SELECT
		funnel_base.lead_id,
	...
	FROM
		(
		SELECT
			mu.full_name AS captain_counsellor,
			ul.contact_id,
			ab.lead_name,
			ul.untouched_flag,
			...
			CASE
				WHEN l.substatus_id = ANY (ARRAY[2,
				3,
				5,
				6,
				57,
			...
			CASE
				WHEN l.substatus_id = ANY (ARRAY[14,
				26,
				27,
				28,
				55,
		...
		FROM
			...
			SELECT
				...
				cm_1.lead_id
			FROM
				contact_mapping cm_1
			WHERE
				cm_1.end_date IS NULL
				AND cm_1.type = 'Sales'::mapping_type) cm
		LEFT JOIN LEAD l ON
			l.contact_id = cm.contact_id
			AND l.rm_id = cm.rm_id
		LEFT JOIN contact c ON
			l.contact_id = c.contact_id
		LEFT JOIN mst_user u ON
			l.rm_id = u.user_id
		LEFT JOIN mst_status ms ON
			l.status_id = ms.status_id
		LEFT JOIN mst_substatus s ON
			l.substatus_id = s.substatus_id
		LEFT JOIN mst_location ml ON
			u.location_id = ml.location_id
		LEFT JOIN mst_user mu ON
			u.reporting_manager_id = mu.user_id
		LEFT JOIN follow_up fu ON
			l.lead_id = fu.lead_id
		LEFT JOIN repeat_lead_logic rll ON
			l.lead_id = rll.lead_id
		LEFT JOIN converted_leads cl ON
			l.lead_id = cl.lead_id
		LEFT JOIN untouched_logic ul ON
			l.lead_id = ul.lead_id
		LEFT JOIN appointment_book ab ON
			l.lead_id = ab.lead_id
		WHERE
			l.is_active
		GROUP BY
			u.full_name,
			...
			ab.workable_sl,
			ab.non_workable_sl,
			ab.appointment_booking_flag) funnel_base
UNION ALL
	SELECT
	...
	FROM
		(
		SELECT
			(ull.log_time AT TIME ZONE 'Asia/Kolkata'::text)::date AS login_date,
			ull.user_id,
		...
		FROM
			user_login_log ull
		JOIN mst_user mu ON
			ull.user_id = mu.user_id 
		LEFT JOIN mst_location ml ON
			mu.location_id = ml.location_id
		GROUP BY
			((ull.log_time AT TIME ZONE 'Asia/Kolkata'::text)::date),
			ull.user_id,
			ml.name,
			mu.full_name
		ORDER BY
			((ull.log_time AT TIME ZONE 'Asia/Kolkata'::text)::date) DESC,
			mu.full_name) login_log
UNION ALL
	SELECT
		NULL::integer AS lead_id,
...
UNION ALL
	...
	FROM
		...
		FROM
			LEAD lll
		LEFT JOIN status_log sl ON
			lll.lead_id = sl.lead_id
			AND (sl.substatus_id = ANY (ARRAY[1,
			2,
			3]))
                        ),
		initial_taken_by AS (
		SELECT
			DISTINCT ON
			(pcm.lead_id) pcm.lead_id,
			pcm.is_taken,
			pcm.rm_id,
			pcm.location_id
		FROM
			pool_contact_mapping pcm
		JOIN new_leads nl_1 ON
			nl_1.lead_id = pcm.lead_id
		WHERE
			pcm.rm_id IS NOT NULL
		ORDER BY
			pcm.lead_id,
			pcm.pool_mapping_id
                        ),
		...
		FROM
			LEAD l
		LEFT JOIN lead_raw lr ON
			lr.lead_raw_id = l.lead_raw_id
                        )
		SELECT
			nl.created_at AS lead_created_date,
			nl.lead_id,
			...
			nl.updated_at_s,
			CASE
				WHEN t.substatus_id <> ALL (ARRAY[1,
				2,
				3]) THEN 1
				ELSE 0
			END AS touched,
			CASE
				WHEN t.substatus_id = ANY (ARRAY[6,
				7,
				13,
				14,
				16,
			...
			END AS workable,
			CASE
				WHEN t.substatus_id = ANY (ARRAY[9,
				10,
				11,
				15,
				56,
			...
			END AS not_pitched,
			CASE
				WHEN t.substatus_id = ANY (ARRAY[29,
				30,
				31,
				32,
				33,
			...
			END AS converted,
			CASE
				WHEN nl.substatus_id_sl <> ALL (ARRAY[1,
				2,
				3]) THEN 1
				ELSE 0
			END AS touched_sl,
			CASE
				WHEN nl.substatus_id_sl = ANY (ARRAY[6,
				7,
				13,
				14,
				16,
			...
			END AS workable_sl,
			CASE
				WHEN nl.substatus_id_sl = ANY (ARRAY[9,
				10,
				11,
				15,
				56,
			...
			CASE
				WHEN nl.substatus_id_sl = ANY (ARRAY[29,
				30,
				31,
				32,
				33,
		...
		FROM
			new_leads nl
		LEFT JOIN contact c ON
			c.contact_id = nl.contact_id
		LEFT JOIN initial_lead_utm ilu ON
			ilu.lead_id = nl.lead_id
		LEFT JOIN till_date_lead t ON
			t.lead_id = nl.lead_id
		LEFT JOIN mst_user mu ON
			mu.user_id = t.rm_id
		LEFT JOIN mst_location ml ON
			ml.location_id = mu.location_id
		LEFT JOIN initial_taken_by pcml ON
			pcml.lead_id = nl.lead_id
		LEFT JOIN mst_user mu2 ON
			mu2.user_id = pcml.rm_id
		LEFT JOIN mst_location ml2 ON
			ml2.location_id = pcml.location_id
		LEFT JOIN mst_status ms ON
			ms.status_id = t.status_id
		LEFT JOIN mst_substatus ms2 ON
			ms2.substatus_id = t.substatus_id
		LEFT JOIN mst_product ms3 ON
			ms3.product_id = t.product_id
		LEFT JOIN mst_subproduct ms4 ON
			ms4.sub_product_id = t.sub_product_ids[array_length(t.sub_product_ids, 1)]) lqr
UNION ALL
        ( WITH reasons AS (
	SELECT
		sl.created_at,
	...
	FROM
		...
		FROM
			status_log sl_1
		WHERE
			sl_1.substatus_id = ANY (ARRAY[10,
			11,
			15,
			56,
			58,
	...
	LEFT JOIN (
		...
		WHERE
			...
			64,
			65]))
				AND sl_1.created_by <> 1
				AND (sl_1.activity_type_id = ANY (ARRAY[1,
				7]))) t ON
		t.lead_id = sl.lead_id
			AND t.created_at >= date_trunc('day'::text, sl.created_at)
				AND t.created_at < (date_trunc('day'::text, sl.created_at) + '1 day'::INTERVAL)
					AND sl.rm_id = t.rm_id
					AND t.status_id = sl.status_id
					AND t.substatus_id = sl.substatus_id
				LEFT JOIN mst_comment mc ON
					mc.comment_id = ANY (t.comment_ids)
				LEFT JOIN mst_user mu ON
					mu.user_id = sl.rm_id
				LEFT JOIN mst_location ml ON
					ml.location_id = mu.location_id
...
LEFT JOIN (
	...
	LEFT JOIN mst_user b ON
		a."reporting manager Id" = b.user_id) people_date ON
	base.counsellor_id = people_date.counsellor_id
LEFT JOIN (
	SELECT
		lead.lead_id,
		lead.contact_id
